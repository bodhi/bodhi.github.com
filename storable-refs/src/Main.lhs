...
[Previously](callback-and-state) we worked through how to persist
state with a callback managed by a framework. My simplified model made
the problem trivial, *by relaxing a key constraint*: The callback is
passed through the GHC FFI in a `FunPtr`.

<aside>"Framework as" in "don't call us, we'll call you!"</aside>
...
<aside>For example, persisting the phase of the last sample of a
fixed-frequency oscillator, so that the next audio buffer can continue
at this phase offset rather than starting at 0 phase for each
buffer</aside>
...
(where `a` is the type of the variable shared between the control
thread and the callback function) as a simplification of
...
I can't even begin to figure out whether we could create a
`Storable` instance for `IORef`!

---

As mentioned previously, <a
href="http://hackage.haskell.org/package/StateVar">StateVar</a>
provides a good-looking abstraction over `IORef` (and others), so
...
<aside>I think <code>StablePtr</code> is needed because we don't
maintain a reference to the
<code>get</code>/<code>set</code>/<code>modify</code> functions on the
non-callback side of the system, and the callback lifecycle is managed
by CoreAudio, not Haskell</aside>
...
Then we need to be able to reconstruct the functions from the
pointers:
