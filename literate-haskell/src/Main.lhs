...
A logbook of setting up [literate programming] for Haskell
experiments.
...
include the front matter. Not sure if this is documented
somewhere... So:
...
and Jekyll doesn't like that syntax. Can we disable it? Yes, by
turning off the `fenced_code_attributes` extension:
...
and Jekyll can show it to us at
<http://localhost:4000/literate-haskell>
...
<aside>Yep, over here. Except on narrow screens, when it's more like "down below, here".</aside>
...
The blank line after `</aside>` has been lost, which confuses
Kramdown, we end up with
...
<aside>I can't actually see these emoji in Emacs, and the reasons for
that are <a
href="https://news.ycombinator.com/item?id=13011185">"hotly
debated"</a>.</aside>

Oh. Next bug is that Pandoc isn't passing `markdown="1"` through, it's
just being stripped. I'll just re-write the previous aside in pure
HTML for now, and try to resolve it later.
...
<aside>Why does the HTML comment not need a leading space here?! Let's
check over there ←</aside>
...
But can't we reuse the target name in the command? Ah, [`$@`](https://www.gnu.org/software/make/manual/html_node/Automatic-Variables.html), bingo!
